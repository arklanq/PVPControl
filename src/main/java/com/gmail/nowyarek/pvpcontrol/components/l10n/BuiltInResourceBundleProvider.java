...
@Singleton
public class BuiltInResourceBundleProvider extends ResourceBundleProvider {
    private final PluginLogger logger;
    private final Provider<String> languageCodeProvider;

    @Inject
    public BuiltInResourceBundleProvider(
        PVPControl plugin,
    ...
    protected Optional<ResourceBundle> createResourceBundle() throws Exception {
        ...
        try {
            return resourceBundle = constructor.constructBuiltInResourceBundle().get();
        } finally {
            this.logger.debug(String.format("Built-in translations ResourceBundle (%s) %s.", languageCodeProvider.get(), resourceBundle.isPresent() ? "loaded" : "not available"));
        }
    }
}
